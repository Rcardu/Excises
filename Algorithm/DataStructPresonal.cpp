...
int SetAllS::Get(int key) {
  ...
  if (time_setall_ > map_[key].second)
    return map_[key].first;
  else
    return value_setall_;
}
bool SetAllS::Constainkey(int key) { return map_.count(key); }
...
void SetAllS::UsSetAllS() {
  ...
  vector<double> priced = clo.RandVector(10, 0.0, 1.0, 73);
  toString(key);
  toString(value);
  for (int i = 0; i < 10; i++) {
    if (priced[i] < 0.25) {
...
double MedianFinder::findMedian() {
  if ((size_max_ + size_min_) % 2 == 1) {
    if (size_max_ > size_min_)
      return (double)Max_.top();
    else
      return (double)Min_.top();
  } else {
...
string AllOne::getMaxKey() { return *tail_->prev->words.begin(); }
string AllOne::getMinKey() { return *head_->prev->words.begin(); }
