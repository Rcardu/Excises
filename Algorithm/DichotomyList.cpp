...
int DichotomyList::flip(int sign){
    return sign^1;
}
int DichotomyList::sign(int num){
...
/// @param n 
/// @return 
bool DichotomyList::isPowerOfTwo(int n){
    return n>0&&n==(n&-n);
}
void DichotomyList::Test_isPowerOfTwo(){
    ...
    for(int i=0;i<10;i++){
        cout<<nums[i]<<endl;
        if(isPowerOfTwo(nums[i]))cout<<"True!";
        else cout<<"False!";
...
void DichotomyList::Test_BitSetComp(){
    ...
    if(BitSetComp(nums,1000))cout<<"True"<<endl;
    else cout<<"False!"<<endl;
}
