...
class AmountData {
 ...
 public:
  ...
  AmountData();
  // 直接传入数据
  AmountData(int n, int m, std::vector<int>& kill, std::vector<int>& blood);
...
class SuperPalindRomesInRange {
 private:
  int superRange_f1(const std::string& left, const std::string& right);
  // 直接用record数组来遍历寻找范
  int superRange_f2(const std::string& left, const std::string& right);
  // 输出所有long类型的超级回文数
  std::vector<long long> superRange_outAll(long long left, long long right);
  // 生成奇数长度的回文
  long long superRange_evenEnlarge(long long seed);
  // 生成偶数长度的回文
  long long superRange_oddEnlarge(long long seed);
  // 判断是否在l，r内，以及是否是回文
  bool superRange_inRand(unsigned long long num, long long l, long long r);
  // 判断你是否是回文
  bool superRange_isPalindrome(long long num);

 public:
  // 直接输入两个字符串
  SuperPalindRomesInRange();
  // 直接给定两个字符串
  SuperPalindRomesInRange(std::string L, std::string R);

 private:
  // 根据算法生成的结果集
  std::string lstr;
  std::string rstr;
  std::vector<long long> record = {1L,
                                   4L,
                                   9L,
                                   121L,
                                   484L,
